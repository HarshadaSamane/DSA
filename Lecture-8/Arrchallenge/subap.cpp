...
// Sarasvati has an array of N non-negative integers. The i-th integer of the array is Ls She
...
//  1<=T<=100.  
//  0<=Ai<=10^9                            //1 sec = 10° operations (approx)
                              
// Test Set 1                            //20 sec =2 x 10° operations (approx)
// 2<= N <= 2000.
...
int main() {
    ...
    int pd = arr[1]-arr[0]; //previous common diff
    int curr = 2; //current arithmetic subarray length
    int ans = 2; //Maximum arithmetic subarray lenth
    int j = 2;
